...
function errorResponse(res, error) {
  ...
  if (message.includes('disabled') || message.includes('schema incompatible') || message.includes('database unavailable')) return res.status(503).json({ ok: false, error: 'communications_temporarily_unavailable' });
