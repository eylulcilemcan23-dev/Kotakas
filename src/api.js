...
apiRouter.get('/migration-status', (_req, res) => {
  res.json({
    ...
    completed: [
      ...
      'dispute workflow with open-dispute payment lock',
      'security audit log adapter and admin audit viewer',
      'dispute participant/admin message history with access control',
      'admin unread notification inbox for dispute activity',
      'CI syntax/role/session/finance/market/admin/communications/smoke tests',
    ],
    next: [
      'staging database schema confirmation',
      'production-compatible listings/disputes/audit/communications migration verification',
      'market, escrow, dispute, communication and audit write activation in staging',
