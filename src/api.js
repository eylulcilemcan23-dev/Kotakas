...
apiRouter.get('/migration-status', (_req, res) => {
  res.json({
    ...
    completed: [
      ...
      'jwt session + legacy login compatibility',
      'google oauth existing-account flow',
      'wallet/commission read adapter',
      'escrow settlement core',
      'CI syntax/role/auth/finance/smoke tests',
    ],
    next: [
      'staging schema verification',
      'transactional wallet hold/release/refund adapter',
      'google auto registration adapter',
      'registration/password reset compatibility',
