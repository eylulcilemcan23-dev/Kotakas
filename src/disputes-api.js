...
function disputeView(row) {
  if (!row) return null;
  return {
    id: String(row.id), orderId: String(row.order_id), openedBy: String(row.opened_by), reason: row.reason,
    status: row.status, resolution: row.resolution || '', resolvedBy: row.resolved_by == null ? null : String(row.resolved_by),
    createdAt: row.created_at, updatedAt: row.updated_at, resolvedAt: row.resolved_at,
  };
}

function resolutionText(value) {
  return ({ refund: 'Tutar alıcıya iade edildi.', release: 'Tutar satıcıya aktarıldı.', dismiss: 'İhtilaf finans hareketi yapılmadan kapatıldı.' })[value] || 'İhtilaf sonuçlandırıldı.';
}
...
export async function detectDisputeCompatibility({ force = false } = {}) {
  ...
  const result = await pool.query(`select column_name from information_schema.columns where table_schema = 'public' and table_name = 'disputes'`);
  const columns = new Set(result.rows.map((row) => row.column_name));
  const blockers = [];
  if (!result.rowCount) blockers.push('missing_table:disputes');
  for (const column of REQUIRED_COLUMNS) if (!columns.has(column)) blockers.push(`missing_column:disputes.${column}`);
...
export async function openDispute({ orderId, openedBy, reason }) {
  ...
  try {
    ...
    const orderResult = await client.query(`select id, buyer_id, seller_id, escrow_state from orders where id = $1 for update`, [order]);
    ...
    const inserted = await client.query(`
      ...
      values ($1, $2, $3, 'open', null, null, now(), now(), null) returning *
...
export async function listUserDisputes(userId, { limit = 50 } = {}) {
  ...
  const result = await pool.query(`
    ...
    where o.buyer_id = $1 or o.seller_id = $1 order by d.id desc limit $2
  `, [user, safeLimit]);
...
export async function listAdminDisputes({ status = '', limit = 50 } = {}) {
  ...
  const params = [];
  let where = '';
  if (safeStatus) { params.push(safeStatus); where = `where d.status = $${params.length}`; }
  params.push(safeLimit);
  const result = await pool.query(`
    select d.*, o.buyer_id, o.seller_id, o.amount, o.escrow_state from disputes d
    join orders o on o.id = d.order_id ${where} order by d.id desc limit $${params.length}
  `, params);
  return result.rows.map((row) => ({ ...disputeView(row), buyerId: String(row.buyer_id), sellerId: String(row.seller_id), amount: Number(row.amount || 0), escrowState: row.escrow_state }));
}
...
export async function resolveDispute({ disputeId, adminId, adminRole, resolution }) {
  ...
  const updated = await pool.query(`
    update disputes set status = 'resolved', resolution = $2, resolved_by = $3, resolved_at = now(), updated_at = now()
    where id = $1 and status = 'open' returning *
  `, [id, safeResolution, admin]);
  const dispute = disputeView(updated.rows[0] || current.rows[0]);
  await writeAudit({ actorId: admin, actorRole: adminRole, action: `dispute_${safeResolution}`, targetType: 'dispute', targetId: id, metadata: { orderId, resolution: safeResolution } })
    .catch((error) => console.error('[KOTAKAS] audit write failed:', error?.message || error));
  await Promise.all([audience.buyer_id, audience.seller_id].filter(Boolean).map((recipient) => createUserNotification({
    userId: recipient, kind: 'dispute_resolved', title: `İhtilaf #${id} sonuçlandırıldı`, body: resolutionText(safeResolution),
    targetType: 'dispute', targetId: id, dedupeKey: `dispute-resolution:${id}:${safeResolution}`, createdBy: admin,
  }).catch(() => null)));
...
function errorResponse(res, error) {
  ...
  if (message.includes('disabled') || message.includes('schema incompatible') || message.includes('database unavailable')) return res.status(503).json({ ok: false, error: 'disputes_temporarily_unavailable' });
...
disputesApiRouter.post('/disputes', requireAuthenticated, async (req, res) => {
  ...
  try {
    ...
    await writeAudit({ actorId: user.id, actorRole: user.role, action: 'dispute_opened', targetType: 'dispute', targetId: dispute.id, metadata: { orderId: dispute.orderId } }).catch(() => {});
    await createAdminNotification({ kind: 'dispute_opened', title: `Yeni ihtilaf #${dispute.id}`, body: dispute.reason.length > 180 ? `${dispute.reason.slice(0, 177)}...` : dispute.reason, targetId: dispute.id, createdBy: user.id }).catch(() => {});
    const audienceResult = await pool.query(`select buyer_id, seller_id from orders where id = $1 limit 1`, [dispute.orderId]);
    const audience = audienceResult.rows[0] || {};
    const otherParticipant = [audience.buyer_id, audience.seller_id].map((value) => value == null ? '' : String(value)).find((value) => value && value !== user.id);
    if (otherParticipant) await createUserNotification({ userId: otherParticipant, kind: 'dispute_opened', title: `İşlem #${dispute.orderId} için ihtilaf açıldı`, body: dispute.reason.length > 220 ? `${dispute.reason.slice(0, 217)}...` : dispute.reason, targetType: 'dispute', targetId: dispute.id, dedupeKey: `dispute-opened:${dispute.id}`, createdBy: user.id }).catch(() => null);
    return res.status(201).json({ ok: true, dispute });
  } catch (error) { return errorResponse(res, error); }
});

disputesApiRouter.get('/disputes/mine', requireAuthenticated, async (req, res) => {
  try { const user = actor(req); return res.json({ ok: true, disputes: await listUserDisputes(user.id, { limit: req.query.limit }) }); }
  catch (error) { return errorResponse(res, error); }
});

disputesApiRouter.get('/admin/disputes', requirePermission(PERMISSIONS.DISPUTES), async (req, res) => {
  try { return res.json({ ok: true, disputes: await listAdminDisputes({ status: req.query.status, limit: req.query.limit }) }); }
  catch (error) { return errorResponse(res, error); }
});
...
disputesApiRouter.post('/admin/disputes/:disputeId/resolve', requirePermission(PERMISSIONS.FINANCE), async (req, res) => {
  ...
  try { const dispute = await resolveDispute({ disputeId: req.params.disputeId, adminId: admin.id, adminRole: admin.role, resolution: req.body?.resolution }); return res.json({ ok: true, dispute }); }
  catch (error) { return errorResponse(res, error); }
});

disputesApiRouter.get('/admin/audit', requirePermission(PERMISSIONS.SECURITY), async (req, res) => {
  try { return res.json({ ok: true, logs: await listAuditLogs({ limit: req.query.limit, action: req.query.action }) }); }
  catch (error) { return errorResponse(res, error); }
});
