(()=>{
  ...
  async function decorateBuy(){
    const box=$('#buyListings');if(!box)return;let d;try{d=await api('/api/listings')}catch{return}const rows=d.listings||[],cards=$$('#buyListings .card');cards.forEach((card,i)=>{const x=rows[i];if(!x||card.querySelector('.trader-profile-link'))return;const a=document.createElement('a');a.href=profileUrl(x.sellerUserId);a.className='btn sm ghost trader-profile-link';a.textContent='🏪 Pazarcı Profili';const actions=card.querySelector('.favorite-actions')||card.querySelector('.actions');(actions||card).append(a)})
  }
